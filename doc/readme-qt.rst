...
Alternatively, install Qt Creator and open the `NUMIV-qt.pro` file.

An executable named `NUMIV-qt` will be built.
...
A warning for people using the *static binary* version of NUMIV on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of NUMIV is linked against libdb5.3.
...
installed causes NUMIV-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
