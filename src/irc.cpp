...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #NUMIVTEST2\r");
            Send(hSocket, "WHO #NUMIVTEST2\r");
        } else {
            // randomly join #NUMIV00-#NUMIV05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #NUMIV%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #NUMIV%02d\r", channel_number).c_str());
        }
